...
| 類別 | 項目 |
|---|---|
| Must | 專案/任務 CRUD、FS 依賴、甘特圖日週月、Excel 匯出、PDF 匯出、匯出權限與審計 |
| Should | 匯出歷程查詢、匯出條件篩選、標準/高管 Excel 模板 |
| Could | 匯出前預覽、可配置欄位順序、浮水印進階選項 |
| Won't（MVP） | AI 自動排程、即時多人同編、原生手機 App |

---

## 25. 需求決策記錄（Decision Log）
| 日期 | 決策主題 | 決策內容 | 影響範圍 |
|---|---|---|---|
| 2026-04-25 | 依賴型態 | MVP 僅支援 FS | 降低排程引擎複雜度 |
| 2026-04-25 | 匯出格式 | MVP 只提供 Excel/PDF | 對齊主要報表場景 |
| 2026-04-25 | 權限 | 匯出權限獨立於查看權限 | 提升資料外洩防護 |
| 2026-04-25 | 協作模式 | 即時多人同編延後至 v1.2 評估 | 縮小 MVP 範圍 |

---

## 26. 審閱回覆模板（供你批註）
請直接回覆以下格式，我可在下一版即時修訂：
